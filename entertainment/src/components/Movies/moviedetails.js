...
export default function MovieDetails() {
  const { castdata, crewdata, moviedatabyid, fetchMovieDetails, user } =
    ...
    <div>
      ...
      >
        ...
        <div className="d-flex" style={{ position: "relative", zIndex: 1 }}>
          <div
            style={{ width: "30%", height: "400px", margin: "3% 0% 0% 10%" }}
          >
            <img
              src={moviedatabyid.image}
              alt={moviedatabyid.moviename}
          ...
          >
            ...
            {new Date(moviedatabyid.release_date) < currentDate ? (
              ...
              >
                <div className="d-flex justify-content-between align-items-center ">
                  ...
                  {user ? (
                    ...
                    <Link to={"/login"}>
                      <button className="btn bg-light">Rate now</button>
                    </Link>
                  )}
                </div>
                <div>
              ...
              >
                <div className="d-flex justify-content-between align-items-center ">
                  <h6>
                    Releasing in{" "}
                    {new Date(moviedatabyid.release_date).toLocaleDateString(
                      "en-GB",
                  ...
                  {user ? (
                    <Link to={"/mrating"}>
                      <button className="btn bg-light">I'm intersted</button>
                    </Link>
                  ) : (
                    <Link to={"/login"}>
                      <button className="btn bg-light">I'm intersted</button>
            ...
            >
              <div className="d-flex align-items-center">
                <h6 className="bg-light text-dark mx-2 p-1">
                  {moviedatabyid.screentype}
                </h6>
                <h6 className="bg-light text-dark p-1">
                  {moviedatabyid.languages}
                </h6>
              </div>
            </div>
            <div
            ...
            <div style={{ borderRadius: "10px", width: "200%" }}>
              {(apiReleaseDate <= currentDate) |
              new Date(
                apiReleaseDate.getTime() - currentDate <=
                  2 * 24 * 60 * 60 * 1000
              ) ? (
                <button
                  className="btn btn-lg px-5 ml-5 bg-danger text-light"
          ...
          >
            <div
              style={{
                ...
                width: "100%",
                height: "480px",
                background: `url(${moviedatabyid.background}) center center / cover no-repeat`,
                opacity: "0.7",
              }}
            ></div>
            <div
      ...
      <Modal show={showModal} onHide={handleCloseModal}>
        ...
        <Modal.Body>
          ...
          {moviedatabyid && moviedatabyid.screentype && (
            <p className="btn mr-5">
              {moviedatabyid.screentype.split(", ").map((type, index) => (
                <React.Fragment key={index}>
                  <Link
                    to={`/movie/${moviedatabyid.id}/${moviedatabyid.moviename}/booking/?type=${type}`}
                  >
                    {type}
                  </Link>
      ...
      <div className="container mt-4">
        ...
        <div>
          ...
          <div className="d-flex">
            {castdata.map((actor, index) => (
              <div key={index}>
                <Link to={`/person/?name=${actor.actor_name}&id=${actor.id}`}>
                  {" "}
                  <img
                ...
                <h6 className="m-3" style={{ width: "50%" }}>
                  {actor.actor_name}
                </h6>
        ...
        <div>
          ...
          {crewdata.map((crew, index) => (
            <div key={index}>
              <Link to={`/persen/?name=${crew.crew_member_name}&id=${crew.id}`}>
                <img
                  src={crew.crew_member_image}
                  alt={crew.crew_member_name}
                  height={100}
                  style={{ borderRadius: "100px" }}
                />
              </Link>
      ...
      <Routes>
        <Route
          path="/mrating"
          element={<PrivateRoute element={<MovieRating />} />}
        />
      </Routes>
    </div>
  );
